...
const EditDelivery = () => {
  ...
  // Load delivery data and options
  useEffect(() => {
    if (id) {
      loadDeliveryData();
    }
  ...
  const loadDeliveryData = async () => {
    try {
      setLoading(true);
      const [deliveryData, customersData, agentsData, provincesData] = await Promise.all([
      ...
      if (deliveryData) {
        // Transform items for the new format
        const items = deliveryData.items?.map((item, index) => ({
          id: index + 1,
          item_name: item.item_name || "",
          unit_cost: parseFloat(item.unit_cost) || 0,
          quantity: parseInt(item.quantity) || 1,
          total_cost: parseFloat(item.total_cost) || (parseFloat(item.unit_cost) || 0) * (parseInt(item.quantity) || 1)
        })) || [];
