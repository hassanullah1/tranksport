...
const EditDelivery = () => {
  ...
  // Item form now includes all fields from AddDelivery
  ...
  const loadDeliveryData = async () => {
    try {
      ...
      if (deliveryData) {
        // Map items with all fields – note database field names
        const items =
          deliveryData.items?.map((item, index) => ({
            id: index + 1,
            item_name: item.item_name || "",
            description: item.item_description || "", // from DB
            agent_cost: parseFloat(item.commission_amount) || 0, // from DB
  ...
  // Same as AddDelivery – sum of agent_cost
  const calculateCommission = () => {
    return formData.items.reduce((sum, item) => {
      return sum + (parseFloat(item.agent_cost) || 0);
    }, 0);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const deliveryData = {
        ...
        // No need to send total_cost or commission_amount – they are derived in backend
      };
  ...
  const translations = {
    header: {
      en: { title: "Edit Delivery" },
      ps: { title: "لیږد سمول" },
      fa: { title: "ویرایش تحویل" },
    },
    buttons: {
      en: {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-lg shadow p-4 mb-4">
              ...
              {/* Item Form Fields – matches AddDelivery */}
